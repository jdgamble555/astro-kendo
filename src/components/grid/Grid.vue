    ...
    <GridPdfExport ref="gridPdfExport">
      ...
      >
        ...
        <template #contactTemplate="{ props }">
          <td v-if="props.rowType !== 'groupHeader'">
            <ContactName :data-item="props.dataItem" />
          </td>
        </template>
        <template #flagTemplate="{ props }">
          <td v-if="props.rowType !== 'groupHeader'" class="text-center">
...
const selectedDataItems = computed(() => {
  return gridData.value.data
    ? gridData.value.data.filter((item) => item.selected === true)
    : [];
});
...
const exportPdfMessage = computed(() => {
  ...
  }).toLanguageString("exportPdf", "Export to PDF");
});

const exportExcelMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("exportExcel", "Export to Excel");
});

const gridSearchMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("gridSearch", "Search in all columns...");
});

const contactNameMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("contactName", "Contact Name");
});

const jobTitleMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("jobTitle", "Job Title");
});

const countryMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("country", "Country");
});

const employeeMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("employee", "Employee");
});

const statusMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("status", "Status");
});

const engagementMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("engagement", "Engagement");
});

const performanceMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("performance", "Performance");
});

const budgetMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("budget", "Budget");
});

const contactsMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("contacts", "Contacts");
});

const phoneMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("phone", "Phone");
});

const addressMessage = computed(() => {
  return provideLocalizationService({
    kendoLocalizationService,
  }).toLanguageString("address", "Address");
});
</script>
